...
import { X, User, Coffee, CreditCard, LogOut } from "lucide-react"
...
export default function Sidebar({ isOpen, onClose }) {
    ...
    return (
        <>
            <div
                className={`fixed inset-0 bg-black/20 backdrop-blur-[2px] z-[998] transition-opacity duration-300 ease-in-out ${isOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"
                    }`}
                onClick={onClose}
            />
            ...
            {/* Sidebar Container */}
            <div
                className={`fixed right-0 top-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-out z-[999] ${isOpen ? "translate-x-0" : "translate-x-full"
                    }`}
            >
                <div className="flex flex-col h-full">
                    ...
                    <div className="p-6 flex items-center justify-between border-b border-gray-100">
                        <div>
                            <h2 className="text-xl font-bold text-gray-800">Kopi Boedaja</h2>
                            <p className="text-xs text-gray-500 mt-1">Menu Pelanggan</p>
                        </div>
                        <button
                            onClick={onClose}
